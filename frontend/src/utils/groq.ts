...
// EARTHZETA's personality and system prompt
const EARTHZETA_PERSONALITY = `You are EARTHZETA, a professional Solana trading assistant with the following traits:
...
Key Information:
- Your token address: ${EARTHZETA_TOKEN_ADDRESS}
- Token URL: https://pump.fun/coin/${EARTHZETA_TOKEN_ADDRESS}
...
export async function streamCompletion(
  ...
  const apiKey = providedApiKey || localStorage.getItem("EARTHZETA_api_key");
  ...
  while (retries < RATE_LIMIT.MAX_RETRIES) {
    try {
          ...
          try {
            ...
            switch (functionName) {
              ...
              case "getWalletBalance":
                ...
                onChunk(
                  `USD Value: $${(
                    balanceResult * (await getSolanaPrice()).price
                  ).toFixed(2)}\n`
                );
                break;
              ...
              case "reviewTransaction":
                ...
                if (txDetails.amount) {
                  onChunk(
                    `Amount: ${(txDetails.amount / LAMPORTS_PER_SOL).toFixed(
                      4
                    )} SOL\n`
                  );
                }
              ...
              case "requestDevnetAirdrop":
                ...
                const airdropResult = await requestDevnetAirdrop(
                  airdropAddress
                );
              ...
              case "swapSolToToken":
                const { amountInSol, outputMint = USDC_MINT } =
      ...
      if (error?.status === 429) {
        logger.warn(
          `Rate limited. Attempt ${retries} of ${RATE_LIMIT.MAX_RETRIES}`
        );
