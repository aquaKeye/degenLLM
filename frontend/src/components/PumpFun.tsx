...
export default function PumpFun({
  ...
  const loadTokenData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://pump.fun/api/token/${EARTHZETA_TOKEN_ADDRESS}`
      );
      ...
      const chartResponse = await fetch(
        `https://pump.fun/api/token/${EARTHZETA_TOKEN_ADDRESS}/chart?period=24h`
      );
  ...
  const loadUserBalance = async () => {
    try {
      ...
      const tokenPublicKey = new PublicKey(EARTHZETA_TOKEN_ADDRESS);
      ...
      const data = await response.json();
      const balance =
        data.tokens.find((t: any) => t.mint === EARTHZETA_TOKEN_ADDRESS)
          ?.amount || 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <a
                href={`https://pump.fun/token/${EARTHZETA_TOKEN_ADDRESS}`}
                target="_blank"
            ...
            {userBalance !== null && (
              <div className="text-right">
                ...
                <p className="text-lg font-medium">
                  {userBalance.toLocaleString()} EARTHZETA
                </p>
              </div>
            )}
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          The EARTHZETA token is a utility token used within the PumpFun
          ecosystem. It allows users to participate in various activities and
          earn rewards.
        </p>
      </div>
    </div>
