...
export class ChatService {
  ...
  private async startAutoMode(): Promise<void> {
    ...
    this.autoModeInterval = setInterval(async () => {
      try {
        ...
        this.recordMessage(
          "assistant",
          `Executed action: ${serviceAnalysis.action}`
        );
  ...
  private async executeAutoAction(
    analysis: ServiceMarketAnalysis
  ): Promise<string> {
    try {
  ...
  private initializeModes(): void {
    ...
    const chatConfig: ModeConfig = {
      ...baseConfig,
      welcomeMessage:
        'Welcome to chat mode! Type "help" for available commands.',
      commands: [
        {
          name: "market",
    ...
    const autoConfig: ModeConfig = {
      ...baseConfig,
      welcomeMessage:
        "Auto mode activated. EARTHZETA will operate autonomously.",
      commands: [
        {
          name: "pause",
  ...
  public async stop(): Promise<void> {
    ...
    this.readline.close();
    console.log("\nGoodbye! EARTHZETA shutting down...");
    process.exit(0);
  }
}
