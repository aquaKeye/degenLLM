...
export class PersistentAgent {
  ...
  async runAutonomousMode(
    interval = CONFIG.AUTOMATION.CONTENT_GENERATION_INTERVAL
  ): Promise<void> {
    ...
    while (true) {
      try {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, CONFIG.AUTOMATION.CONTENT_GENERATION_INTERVAL)
        );
      }
    }
  }
...
export const createPersistentAgent = async (
  ...
  if (!groqApiKey || !postgresUrl) {
    throw new Error(
      "Missing required environment variables: GROQ_API_KEY or POSTGRES_URL"
    );
  }
