...
export const CONFIG = {
  SOLANA: {
    ...
    TOKEN_SETTINGS: {
      ...
      SYMBOL: "SOL", // Add this
      DECIMALS: 9, // Add this
      PRICE_UPDATE_INTERVAL: parseInt(
        process.env.PRICE_UPDATE_INTERVAL || "300000",
        10
      ),
      MARKET_UPDATE_INTERVAL: parseInt(
        process.env.MARKET_UPDATE_INTERVAL || "600000",
        10
      ),
      DATA_PROVIDERS: {
        BIRDEYE: {
  ...
  AI: {
    GROQ: {
      ...
      DEFAULT_TEMPERATURE: parseFloat(
        process.env.GROQ_DEFAULT_TEMPERATURE || "0.7"
      ),
      THREAD_ID: "EARTHZETA_v1", // Add THREAD_ID property
      SYSTEM_PROMPTS: {
        MARKET_ANALYSIS:
          "Analyze the following market data and provide insights:",
        TRADE_DECISION:
          "Based on the market analysis, recommend trading actions:",
        CONTENT_GENERATION:
          "Generate market commentary based on the following data:",
        MEME_GENERATION:
          "Create engaging crypto meme content based on market data:",
      },
    },
  },
  ...
  SOCIAL: {
    TWITTER: {
      ...
      POSTING: {
        ...
        PRICE_CHANGE_THRESHOLD: parseFloat(
          process.env.TWEET_PRICE_CHANGE_THRESHOLD || "0.05"
        ),
      },
    },
  },
  ...
  AUTOMATION: {
    ...
    CONTENT_GENERATION_INTERVAL: parseInt(
      getRequiredEnvVar("CONTENT_GENERATION_INTERVAL", "120000")
    ),
    MARKET_MONITORING_INTERVAL: parseInt(
      getRequiredEnvVar("MARKET_MONITORING_INTERVAL", "30000")
    ),
    COMMUNITY_ENGAGEMENT_INTERVAL: parseInt(
      getRequiredEnvVar("COMMUNITY_ENGAGEMENT_INTERVAL", "300000")
    ),
  },
